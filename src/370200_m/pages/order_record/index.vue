<template>
  <div>
    ...
    <div>
      <div v-if="!isList">
        <van-empty image="search" :description="isList_msg" />
      </div>
...
<script>
import { DropdownMenu, DropdownItem, Dialog, Empty } from 'vant';
  ...
  mounted() {
    ...
    queryOrderRecord({
      ...
      if (data.code === 0) {
        ...
        a.details.length > 0 ? (this.isList = true) : (this.isList = false);
        this.isList_msg = '该搜索无信息';
      } else {
        this.isList = false;
        this.isList_msg = data.msg;
        this.model = [];
        Dialog.alert({
          message: data.msg,
        }).then(() => {
